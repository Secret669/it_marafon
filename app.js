/**
 * app.js — головний модуль АгроФорми
 * Підключає api.js для роботи з сервером (або localStorage як fallback).
 * Підтримує динамічні рядки в таблицях.
 */
...
let currentTab    = 0;
let currentForm   = null;   // кешований об'єкт форми

const MODULES = {
  crop:  { label: 'Рослинництво',       icon: '🌱', tabs: ['Загальна інформація', 'Маржинальний дохід', 'Основні засоби'] },
  swine: { label: 'Відгодівля свиней',  icon: '🐷', tabs: ['МД — Таблиця 1', 'Прибуток — Таблиця 2', 'Аналіз'] },
  bulls: { label: 'Розведення бичків',  icon: '🐂', tabs: ['МД — Таблиця 1', 'Агрегація — Таблиця 2', 'Прибуток — Таблиця 3'] },
  dairy: { label: 'Молочне скотарство', icon: '🐄', tabs: ['МД — Таблиця 1', 'Агрегація — Таблиця 2', 'Прибуток — Таблиця 3'] },
};
...
async function openForm(id) {
  ...
  // Нормалізуємо data (може прийти як рядок від MySQL)
...
// ============================================================
// COLLECT DATA (scalar fields + dynamic tables)
// ============================================================
...
function renderTab() {
  ...
  const builders = {
    crop:  [buildCropGeneral, buildCropGM,   buildCropAssets],
...
// ── Combobox (async: завантажує опції) ───────────────────────────
...
// ============================================================
/**
 * dynTable(section, rows, columns, opts)
 *   section — ключ в data (рядки зберігаються як data[section])
 *   rows    — поточний масив рядків
 *   columns — [{ key, label, type:'text'|'number'|'calc', width?, def? }]
 *   opts    — { addLabel, calcRow }
 */
function dynTable(section, rows, columns, opts = {}) {
  ...
  return `<table class="table-section">
...
window.dynAddRow = function(section) {
  ...
  // Клонуємо структуру колонок з першого рядка (якщо є), інакше порожній рядок
...
// CSS для кнопок динамічних рядків (вставляємо програмно один раз)
...
async function buildCropGeneral(body, data) {
  ...
  // Заповнюємо combobox-и після рендеру
...
async function buildCropGM(body, data) {
  ...
  ${sectionWrap('🌾 Назва та параметри культури', `
    ...
    ${row('Площа', numField('gm_area', data), 'га')}
    <div id="ph-gm_curr-row"></div>
  `, 'blue')}

  ${sectionWrap('💰 Валова виручка', `
    <table class="table-section">
      <tr><th>Продукція</th><th>Од.</th><th>Врожайність</th><th>Ціна/од.</th><th>Сума (авто)</th></tr>
      <tr>
        <td>Основна продукція</td>
        <td><input data-field="gm_p1_unit"  value="${fillField(data,'gm_p1_unit','ц')}"  oninput="recalcAll()"></td>
        <td><input type="number" data-field="gm_p1_qty"   value="${fillField(data,'gm_p1_qty','')}"   step="any" oninput="recalcAll()"></td>
        <td><input type="number" data-field="gm_p1_price" value="${fillField(data,'gm_p1_price','')}" step="any" oninput="recalcAll()"></td>
        <td><input data-field="gm_p1_sum" readonly class="calc"></td>
      </tr>
      <tr>
        <td>Побічна продукція 1</td>
        <td><input data-field="gm_p2_unit"  value="${fillField(data,'gm_p2_unit','ц')}"  oninput="recalcAll()"></td>
        <td><input type="number" data-field="gm_p2_qty"   value="${fillField(data,'gm_p2_qty','')}"   step="any" oninput="recalcAll()"></td>
        <td><input type="number" data-field="gm_p2_price" value="${fillField(data,'gm_p2_price','')}" step="any" oninput="recalcAll()"></td>
        <td><input data-field="gm_p2_sum" readonly class="calc"></td>
      </tr>
      <tr>
        <td>Субсидії / дотації</td><td>га</td>
        <td><input type="number" data-field="gm_subsidy_qty"   value="${fillField(data,'gm_subsidy_qty','')}"   step="any" oninput="recalcAll()"></td>
        <td><input type="number" data-field="gm_subsidy_price" value="${fillField(data,'gm_subsidy_price','')}" step="any" oninput="recalcAll()"></td>
        <td><input data-field="gm_subsidy_sum" readonly class="calc"></td>
      </tr>
    </table>
  ...
  ${sectionWrap('🌱 Насіння', `
    ${dynTable('gm_seeds', dynRows(data,'gm_seeds',[{name:'',unit:'кг',qty:'',price:'',sum:''}]), [
      ...
      { key:'unit',  label:'Од.',         type:'text', width:'60px', def:'кг' },
  ...
  ${sectionWrap('🧪 Добрива', `
    <table class="table-section">
      <tr><th>Речовина</th><th>кг/ц осн.</th><th>кг/ц поб.</th><th>К-сть (авто)</th><th>Ціна/кг</th><th>Сума (авто)</th></tr>
      ${[['N','1.2','0.4'],['P₂O₅','1','1'],['K₂O','1','1']].map(([nm,dm,ds])=>`<tr>
        <td>${nm}</td>
        <td><input type="number" data-field="gm_${nm.replace('₂','2').replace('₅','5')}_main" value="${fillField(data,'gm_'+nm.replace('₂','2').replace('₅','5')+'_main',dm)}" step="any" oninput="recalcAll()"></td>
        <td><input type="number" data-field="gm_${nm.replace('₂','2').replace('₅','5')}_side" value="${fillField(data,'gm_'+nm.replace('₂','2').replace('₅','5')+'_side',ds)}" step="any" oninput="recalcAll()"></td>
        <td><input data-field="gm_${nm.replace('₂','2').replace('₅','5')}_qty"  readonly class="calc"></td>
        <td><input type="number" data-field="gm_${nm.replace('₂','2').replace('₅','5')}_price" value="${fillField(data,'gm_'+nm.replace('₂','2').replace('₅','5')+'_price','')}" step="any" oninput="recalcAll()"></td>
        <td><input data-field="gm_${nm.replace('₂','2').replace('₅','5')}_sum"  readonly class="calc"></td>
      </tr>`).join('')}
    </table>
  ...
  ${sectionWrap('🌿 Засоби захисту рослин (ЗЗР)', `
    ${dynTable('gm_zzr', dynRows(data,'gm_zzr',[{name:'',unit:'л/га',qty:'',price:'',sum:''}]), [
      ...
      { key:'unit',  label:'Од.',         type:'text', width:'60px', def:'л/га' },
      { key:'qty',   label:'К-сть/га',    type:'number' },
      { key:'price', label:'Ціна/од.',    type:'number' },
      { key:'sum',   label:'Сума (авто)', type:'calc'   },
    ])}
  ...
  ${sectionWrap('⚙️ Механізація та інші витрати', `
    ${row('Послуги механізації підрядників', numField('gm_mech_contract',data), curr)}
    ${row('Змінні витрати власної механізації', numField('gm_mech_own',data), curr)}
    ${row('Ремонт основних засобів',          numField('gm_mech_repair',data), curr)}
    ${row('Сушка',                           numField('gm_dry',data), curr)}
  ...
  ${sectionWrap('📊 Підсумок МД', `
    ...
    ${row('<strong>Маржинальний дохід</strong>', numField('gm_md',data,'',true), curr)}
    ${row('МД на 1 га',                         numField('gm_md_per_ha',data,'',true), curr+'/га')}
    ${row('Потреба в капіталі (об. активи ×0.6)', numField('gm_capital',data,'',true), curr)}
  `)}`;

  // Async combobox placeholders
  for (const [field, key, ph] of [['gm_crop','crops','пшениця, кукурудза…'],['gm_curr','currencies','грн, €, $']]) {
    const ph_el = document.getElementById('ph-' + field) || document.getElementById('ph-' + field + '-row');
    if (ph_el) {
      const html = await cbHtml(field, data, key, ph);
      if (field === 'gm_crop') ph_el.outerHTML = row('Назва культури', html);
      else                      ph_el.outerHTML = row('Грошова одиниця', html);
    }
  }
}
...
function buildSwineF1(body, data) {
  body.innerHTML = `
  <div style="margin-bottom:12px;font-size:12px;color:var(--gray)">Форма 1 — МД. Два варіанти відгодівлі для порівняння.</div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
  ${['1','2'].map(v => `<div>
    ${sectionWrap(`🐷 Свині на відгодівлі ${v}`, `
      ${row('Вага поросяти (поч.)',     numField('sw'+v+'_w_start',data), 'кг')}
      ${row('Вага реалізації (кін.)',   numField('sw'+v+'_w_end',data), 'кг')}
      ${row('Загальний приріст',        numField('sw'+v+'_gain',data,'',true), 'кг')}
      ${row('Тривалість відгодівлі',    numField('sw'+v+'_days_fat',data), 'днів')}
      ${row('Санітарні заходи',        numField('sw'+v+'_days_san',data), 'днів')}
      ...
      ${row('Вихід туші',              numField('sw'+v+'_yield',data), '%')}
      ${row('Товарна маса/голову',     numField('sw'+v+'_mkt_w',data,'',true), 'кг')}
      ${row('К-сть голів',            numField('sw'+v+'_heads',data), 'гол')}
      ${row('Ціна реалізації',         numField('sw'+v+'_price',data), 'грн/кг')}
      ${row('Виручка від свиней',      numField('sw'+v+'_rev',data,'',true), 'грн')}
      ${row('Субсидії',               numField('sw'+v+'_subsidy',data), 'грн')}
      ...
      <div style="font-size:12px;color:var(--gray);margin:6px 0 2px">Корми:</div>
      ${dynTable('sw'+v+'_feeds', dynRows(data,'sw'+v+'_feeds',[{name:'',unit:'ц',qty:'',price:'',sum:''}]), [
        { key:'name',label:'Назва корму',type:'text'},
        { key:'unit',label:'Од.',type:'text',width:'50px',def:'ц'},
        { key:'qty', label:'К-сть',type:'number'},
        { key:'price',label:'Ціна/од.',type:'number'},
        { key:'sum', label:'Сума (авто)',type:'calc'},
      ], {addLabel:'+ Корм'})}
      ${row('Корми, всього',           numField('sw'+v+'_feed_total',data,'',true), 'грн')}
      ${row('Ветеринар, медикаменти',  numField('sw'+v+'_vet',data), 'грн')}
      ${row('Механізація',            numField('sw'+v+'_mech',data), 'грн')}
      ${row('Роботи і послуги',        numField('sw'+v+'_services',data), 'грн')}
      ${row('Інші',                   numField('sw'+v+'_other',data), 'грн')}
      ${row('<strong>Змінні витрати, всього</strong>', numField('sw'+v+'_costs',data,'',true), 'грн')}
      ${row('<strong>Маржинальний дохід</strong>',     numField('sw'+v+'_md',data,'',true), 'грн')}
      ${row('МД на 1 місце в рік',    numField('sw'+v+'_md_per',data,'',true), 'грн/міс.')}
    `,'blue')}
  </div>`).join('')}
  </div>`;
}
...
function buildSwineF2(body, data) {
  body.innerHTML = sectionWrap('💹 Прибуток — Зведена таблиця', `
    ${row('Отримано продукції',         numField('swp_prod',data),       'кг заб.маси/гол')}
    ${row('Ціна',                      numField('swp_price',data),      'грн/кг')}
    ${row('Субсидії',                  numField('swp_sub',data),        'грн/гол')}
    ${row('Пропорційні змінні витрати', numField('swp_var',data),        'грн/гол')}
    <hr style="margin:8px 0">
    ${row('ЗП (наймані)',              numField('swp_labor_hire',data),  'грн/люд-год')}
    ${row('ЗП (сімейна)',              numField('swp_labor_fam',data),   'грн/люд-год')}
    ${row('Оренда землі',              numField('swp_land_rent',data),   'грн/гол')}
    ${row('Техніка (варт. придб.)',    numField('swp_mach_cost',data),   'грн/гол')}
    ${row('Будинки (варт. придб.)',    numField('swp_build_cost',data),  'грн/гол')}
    ${row('Накладні витрати',          numField('swp_overhead',data),    'грн/гол')}
    ${row('Інші постійні',            numField('swp_fixed_other',data),  'грн/гол')}
    <hr style="margin:8px 0">
    ${row('<strong>Сукупні витрати</strong>',       numField('swp_total_cost',data,'',true), 'грн/гол')}
    ${row('<strong>Підприємницький дохід</strong>', numField('swp_ent_inc',data,'',true), 'грн/гол')}
    ${row('<strong>Прибуток</strong>',              numField('swp_profit',data,'',true), 'грн/гол')}
  `,'green');
}

function buildSwineAnalysis(body, data) {
  body.innerHTML = sectionWrap('📊 Аналіз беззбитковості', `
    ${row('Поріг беззбитковості по ціні',        numField('swa_bep_price',data,'',true), 'грн/кг')}
    ${row('Поріг беззбитковості по кількості',   numField('swa_bep_qty',data,'',true),   'кг/гол')}
    ${row('Коефіцієнт використання місць',       numField('swa_util',data,'',true),      '%')}
    ${row('Потреба в оборотному капіталі',       numField('swa_working_cap',data,'',true),'грн')}
    ${row('Потреба в основному капіталі',        numField('swa_fixed_cap',data),          'грн')}
  `,'blue');
}
...
function buildBullsMD(body, data) {
  ...
  ${['I','II'].map((v,vi) => `<div>
    ${sectionWrap(`🐂 Бички ${v}`, `
      ...
      ${row('Кінцева маса',               numField('bl'+vi+'_w_end',data), 'кг')}
      ${row('Загальний приріст',           numField('bl'+vi+'_gain',data,'',true), 'кг')}
      ${row('Вік на початку',             numField('bl'+vi+'_age_start',data), 'тижнів')}
      ${row('Вік по закінченню',          numField('bl'+vi+'_age_end',data), 'місяців')}
      ${row('Тривалість',                 numField('bl'+vi+'_duration',data,'',true), 'днів')}
      ${row('Середньодобовий приріст',     numField('bl'+vi+'_adg',data,'',true), 'г/день')}
      ${row('Вихід м\'яса',               numField('bl'+vi+'_yield',data), '%')}
      ${row('К-сть тварин',              numField('bl'+vi+'_heads',data), 'гол/од')}
      ${row('К-сть УГ',                  numField('bl'+vi+'_lu',data,'',true), 'УГ/од')}
      ${row('Ціна реалізації',            numField('bl'+vi+'_price',data), 'грн/кг ЖМ')}
      ${row('Виручка',                   numField('bl'+vi+'_rev',data,'',true), 'грн')}
      ...
      ${dynTable('bl'+vi+'_feeds', dynRows(data,'bl'+vi+'_feeds',[
        {name:'Сухе молоко',unit:'ц',qty:'',price:'',sum:''},
        {name:'Корм для телят',unit:'ц',qty:'',price:'',sum:''},
        {name:'Концентр. корм I',unit:'ц',qty:'',price:'',sum:''},
      ]), [
        { key:'name',label:'Назва корму',type:'text'},
        { key:'unit',label:'Од.',type:'text',width:'50px',def:'ц'},
        { key:'qty', label:'К-сть',type:'number'},
      ...
      ${row('Ветеринар',                 numField('bl'+vi+'_vet',data), 'грн')}
      ${row('Механізація',              numField('bl'+vi+'_mech',data), 'грн')}
      ${row('Інші',                     numField('bl'+vi+'_other',data), 'грн')}
      ${row('<strong>Змінні витрати</strong>',  numField('bl'+vi+'_costs',data,'',true), 'грн')}
      ${row('<strong>Маржинальний дохід</strong>', numField('bl'+vi+'_md',data,'',true), 'грн')}
      ${row('МД на 1 місце',            numField('bl'+vi+'_md_per',data,'',true), 'грн/міс')}
    `,'blue')}
  </div>`).join('')}
  </div>`;
}
...
function buildBullsAgg(body, data) {
  body.innerHTML = sectionWrap('🔗 Агрегація кормовиробництва і тваринництва', `
    ${dynTable('agg_bulls', dynRows(data,'agg_bulls',[
      {name:'Бички на відгодівлі',heads:'',area:'',ration:'',feed:'',rev:'',cost:'',md:''},
      {name:'Кукурудза на силос', heads:'',area:'',ration:'',feed:'',rev:'',cost:'',md:''},
      {name:'Люцерна на сінаж',  heads:'',area:'',ration:'',feed:'',rev:'',cost:'',md:''},
      {name:'Люцерна на сіно',   heads:'',area:'',ration:'',feed:'',rev:'',cost:'',md:''},
    ]), [
      { key:'name',   label:'Виробництво',    type:'text'   },
      { key:'heads',  label:'Голів',           type:'number' },
      { key:'area',   label:'Площа, га',       type:'number' },
      { key:'ration', label:'Раціон, %',       type:'number' },
      { key:'feed',   label:'Корми МДж',       type:'number' },
      { key:'rev',    label:'Тов.прод., грн',  type:'number' },
      { key:'cost',   label:'Витрати, грн',    type:'number' },
      { key:'md',     label:'МД, грн',         type:'calc'   },
    ], {addLabel:'+ Позиція'})}
    ${row('<strong>Разом</strong>', numField('agg_bulls_md_tot',data,'',true), 'грн')}
  `,'green');
}

function buildBullsProfit(body, data) {
  body.innerHTML = sectionWrap('💹 Сукупні витрати та прибуток', `
    ${row('Отримано продукції',              numField('bp_prod',data),       'кг заб.маси/гол')}
    ${row('Ціна',                           numField('bp_price',data),      'грн/кг')}
    ${row('Субсидії',                       numField('bp_sub',data),        'грн/гол')}
    ${row('Пропорційні змінні витрати',      numField('bp_var',data,'',true),'грн/гол')}
    ${row('Відс. ставка (власн. кап.)',     numField('bp_int_own',data,'15'),'%')}
    ${row('Відс. ставка (залучений)',        numField('bp_int_hire',data,'17'),'%')}
    ${row('ЗП (сімейна)',                   numField('bp_wage_fam',data,'2.5'),'грн/люд-год')}
    ${row('ЗП (наймані)',                   numField('bp_wage_hire',data,'2.8'),'грн/люд-год')}
    ${row('Техніка: варт. придб.',          numField('bp_mach',data,'1250'), 'грн/гол')}
    ${row('Будівлі: варт. придб.',         numField('bp_build',data,'2501'), 'грн/гол')}
    ${row('Інші постійні',                 numField('bp_fixed_spec',data,'25'),'грн/гол')}
    ${row('Накладні витрати',              numField('bp_overhead',data,'18'), 'грн/гол')}
    <hr style="margin:8px 0">
    ${row('<strong>Сукупні витрати</strong>',       numField('bp_total_cost',data,'',true),'грн/гол')}
    ${row('<strong>Підприємницький дохід</strong>', numField('bp_ent_inc',data,'',true),'грн/гол')}
    ${row('<strong>Прибуток</strong>',              numField('bp_profit',data,'',true),'грн/гол')}
  `,'green');
}
...
function buildDairyMD(body, data) {
  ...
  ${['І','ІІ'].map((v,vi) => `<div>
    ${sectionWrap(`🐄 Молочна худоба ${v}`, `
      ...
      ${row('УГ',                          numField('dy'+vi+'_lu',data,'',true), 'УГ')}
      ...
      ${row('Міжотельний (років)',          numField('dy'+vi+'_calving_yr',data,'',true), 'років')}
      ${row('К-сть телят на рік',          numField('dy'+vi+'_calves',data,'',true), 'гол/рік')}
      ...
      ${row('Жирність / Білок',            numField('dy'+vi+'_fat',data,vi===0?'4.1':'') + ' / ' + numField('dy'+vi+'_prot',data,vi===0?'3.6':''), '%')}
      <hr style="margin:8px 0">
      ${row('Ціна молока',                 numField('dy'+vi+'_milk_price',data), 'грн/кг')}
      ${row('Виручка від молока',          numField('dy'+vi+'_milk_rev',data,'',true), 'грн')}
      ${row('Ціна теляти',                 numField('dy'+vi+'_calf_price',data), 'грн/гол')}
      ${row('Виручка від телят',           numField('dy'+vi+'_calf_rev',data,'',true), 'грн')}
      ${row('Вихід (вибракувані корови)',   numField('dy'+vi+'_cull_yield',data,'55'), '%')}
      ${row('Ціна вибракуваної корови',    numField('dy'+vi+'_cull_price',data), 'грн/кг')}
      ${row('Виручка від вибракуваних',    numField('dy'+vi+'_cull_rev',data,'',true), 'грн')}
      ${row('Субсидії',                   numField('dy'+vi+'_subsidy',data), 'грн')}
      ...
      <div style="font-size:12px;color:var(--gray);margin:6px 0 2px">Покупні корми:</div>
      ${dynTable('dy'+vi+'_feeds', dynRows(data,'dy'+vi+'_feeds',[
        {name:'ЗЦМ',unit:'ц',qty:'',price:'',mj:'713',sum:''},
        {name:'Фураж. ячмінь',unit:'ц',qty:'',price:'',mj:'713',sum:''},
        {name:'Соєвий шрот',unit:'ц',qty:'',price:'',mj:'705',sum:''},
      ]), [
        { key:'name',label:'Назва корму',type:'text'},
        { key:'unit',label:'Од.',type:'text',width:'45px',def:'ц'},
        { key:'mj',  label:'МДж ЧЕЛ',type:'number',width:'70px'},
        { key:'qty', label:'К-сть',type:'number'},
        { key:'price',label:'Ціна/од.',type:'number'},
        { key:'sum', label:'Сума (авто)',type:'calc'},
      ], {addLabel:'+ Корм'})}
      ...
      ${row('Прибирання гною',            numField('dy'+vi+'_manure',data), 'грн')}
      ...
      ${row('Інші',                      numField('dy'+vi+'_other',data), 'грн')}
      ${row('<strong>Змінні витрати, всього</strong>', numField('dy'+vi+'_costs',data,'',true), 'грн')}
      ${row('<strong>Маржинальний дохід</strong>',     numField('dy'+vi+'_md',data,'',true), 'грн')}
    `,'blue')}
  </div>`).join('')}
  </div>`;
}
...
function buildDairyProfit(body, data) {
  body.innerHTML = sectionWrap('💹 Сукупні витрати та прибуток', `
    ${row('Отримано молока',                 numField('dp_milk',data), 'кг/гол/рік')}
    ${row('Ціна молока',                    numField('dp_price',data), 'грн/кг')}
    ${row('Субсидії',                       numField('dp_sub',data), 'грн/гол')}
    ${row('Пропорційні змінні витрати',      numField('dp_var',data,'',true),'грн/гол')}
    ${row('Відс. ставка (власн. кап.)',     numField('dp_int_own',data,'15'),'%')}
    ${row('Відс. ставка (залучений)',        numField('dp_int_hire',data,'17'),'%')}
    ${row('ЗП (сімейна)',                   numField('dp_wage_fam',data,'2.5'),'грн/люд-год')}
    ${row('ЗП (наймані)',                   numField('dp_wage_hire',data,'2.8'),'грн/люд-год')}
    ${row('Техніка: варт. придб.',         numField('dp_mach',data), 'грн/гол')}
    ${row('Будівлі: варт. придб.',        numField('dp_build',data), 'грн/гол')}
    ${row('Інші постійні витрати',         numField('dp_fixed_other',data,'25'),'грн/гол')}
    ${row('Накладні витрати',             numField('dp_overhead',data,'18'),'грн/гол')}
    <hr style="margin:8px 0">
    ${row('<strong>Сукупні витрати</strong>',       numField('dp_total_cost',data,'',true),'грн/гол')}
    ${row('<strong>Підприємницький дохід</strong>', numField('dp_ent_inc',data,'',true),'грн/гол')}
    ${row('<strong>Прибуток</strong>',              numField('dp_profit',data,'',true),'грн/гол')}
  `,'green');
}
...
function setCalc(field, value) {
  ...
  if (el) el.value = isNaN(value)||!isFinite(value) ? '' : value.toFixed(2);
}
function getVal(field) {
  const el = document.querySelector(`[data-field="${field}"]`);
  return el ? fnum(el.value) : 0;
}
// Зчитати суму стовпця динамічної таблиці
function dynColSum(section, col) {
  let s = 0;
  document.querySelectorAll(`[data-dyn-section="${section}"] [data-col="${col}"]`).forEach(el => { s += fnum(el.value); });
  return s;
}
// Прорахувати колонку 'sum' = qty * price у динамічній таблиці
...
function recalcCropGM() {
  ...
  const p2  = getVal('gm_p2_qty') * getVal('gm_p2_price');
  const sub = getVal('gm_subsidy_qty') * getVal('gm_subsidy_price');
  setCalc('gm_p1_sum', p1); setCalc('gm_p2_sum', p2); setCalc('gm_subsidy_sum', sub);
  const grossRev = p1 + p2 + sub;
  setCalc('gm_gross_rev', grossRev);

  const seedTotal = dynCalcSum('gm_seeds');
  setCalc('gm_seed_total', seedTotal);

  const mainQty = getVal('gm_p1_qty'), sideQty = getVal('gm_p2_qty');
  let fertTotal = 0;
  for (const [nm] of [['N'],['P2O5'],['K2O']]) {
    const nQty = getVal(`gm_${nm}_main`)*mainQty + getVal(`gm_${nm}_side`)*sideQty;
  ...
  setCalc('gm_md', md); setCalc('gm_md_per_ha', md/area); setCalc('gm_capital', varTotal*0.6);
  ...
  if (mdPer) { mdPer.textContent = 'на 1 га: '+fmt(md/area)+' грн/га'; }
}
...
function recalcSwine() {
  for (const v of ['1','2']) {
    const gain=getVal('sw'+v+'_w_end')-getVal('sw'+v+'_w_start');
    const cycle=getVal('sw'+v+'_days_fat')+getVal('sw'+v+'_days_san');
    const adg=getVal('sw'+v+'_days_fat')>0?gain*1000/getVal('sw'+v+'_days_fat'):0;
    const mktW=getVal('sw'+v+'_w_end')*getVal('sw'+v+'_yield')/100;
    setCalc('sw'+v+'_gain',gain); setCalc('sw'+v+'_cycle',cycle);
    setCalc('sw'+v+'_adg',adg); setCalc('sw'+v+'_turns',cycle>0?365/cycle:0);
    setCalc('sw'+v+'_mkt_w',mktW);
    const heads=getVal('sw'+v+'_heads');
    const rev=mktW*heads*getVal('sw'+v+'_price');
    setCalc('sw'+v+'_rev',rev);
    setCalc('sw'+v+'_gross',rev+getVal('sw'+v+'_subsidy'));
    const feedTotal=dynCalcSum('sw'+v+'_feeds');
    const costs=getVal('sw'+v+'_piglet_cost')*heads+feedTotal+getVal('sw'+v+'_vet')+getVal('sw'+v+'_mech')+getVal('sw'+v+'_services')+getVal('sw'+v+'_other');
    const md=getVal('sw'+v+'_gross')-costs;
    setCalc('sw'+v+'_feed_total',feedTotal); setCalc('sw'+v+'_costs',costs);
    setCalc('sw'+v+'_md',md); setCalc('sw'+v+'_md_per',heads>0?md/heads:0);
  }
}

function recalcBulls() {
  for(let vi=0;vi<2;vi++){
    const wE=getVal('bl'+vi+'_w_end'), gain=wE-getVal('bl'+vi+'_w_start');
    const dur=getVal('bl'+vi+'_age_end')*30-getVal('bl'+vi+'_age_start')*7;
    setCalc('bl'+vi+'_gain',gain); setCalc('bl'+vi+'_duration',dur>0?dur:0);
    setCalc('bl'+vi+'_adg',dur>0?gain*1000/dur:0); setCalc('bl'+vi+'_lu',wE/500);
    const heads=getVal('bl'+vi+'_heads');
    const rev=wE*(getVal('bl'+vi+'_yield')/100)*heads*getVal('bl'+vi+'_price');
    setCalc('bl'+vi+'_rev',rev);
    setCalc('bl'+vi+'_gross',rev+getVal('bl'+vi+'_subsidy'));
    const feedTotal=dynCalcSum('bl'+vi+'_feeds');
    setCalc('bl'+vi+'_feed_total',feedTotal);
    const costs=getVal('bl'+vi+'_calf_cost')*heads+feedTotal+getVal('bl'+vi+'_vet')+getVal('bl'+vi+'_mech')+getVal('bl'+vi+'_other');
    const md=getVal('bl'+vi+'_gross')-costs;
    setCalc('bl'+vi+'_costs',costs); setCalc('bl'+vi+'_md',md); setCalc('bl'+vi+'_md_per',heads>0?md/heads:0);
  }
}
...
function recalcBullsAgg() {
  ...
  document.querySelectorAll('[data-dyn-section="agg_bulls"] tr[data-row-idx]').forEach(tr=>{
    const r=fnum(tr.querySelector('[data-col="rev"]')?.value), c=fnum(tr.querySelector('[data-col="cost"]')?.value);
    const mdEl=tr.querySelector('[data-col="md"]'); if(mdEl) mdEl.value=(r-c).toFixed(2); mTot+=r-c;
  });
  setCalc('agg_bulls_md_tot',mTot);
}

function recalcBullsProfit() {
  const rev=getVal('bp_prod')*getVal('bp_price')+getVal('bp_sub'), varC=getVal('bp_var');
  const cap=(getVal('bp_mach')+getVal('bp_build'))/2*(getVal('bp_int_own')/100*0.5+getVal('bp_int_hire')/100*0.5);
  const tot=varC+cap+getVal('bp_fixed_spec')+getVal('bp_overhead');
  setCalc('bp_total_cost',tot); setCalc('bp_ent_inc',rev-tot); setCalc('bp_profit',rev-tot);
}

function recalcDairy() {
  for(let vi=0;vi<2;vi++){
    const lw=getVal('dy'+vi+'_lw'), calvInt=getVal('dy'+vi+'_calving_int');
    const calvYr=calvInt>0?365/calvInt:0;
    const calves=calvYr*(1-getVal('dy'+vi+'_calf_loss')/100);
    setCalc('dy'+vi+'_lu',lw/500); setCalc('dy'+vi+'_calving_yr',calvYr); setCalc('dy'+vi+'_calves',calves);
    ...
    const cullRev=getVal('dy'+vi+'_replace_pct')/100*lw*(getVal('dy'+vi+'_cull_yield')/100)*getVal('dy'+vi+'_cull_price');
    setCalc('dy'+vi+'_milk_rev',milkRev); setCalc('dy'+vi+'_calf_rev',calfRev); setCalc('dy'+vi+'_cull_rev',cullRev);
    const gross=milkRev+calfRev+cullRev+getVal('dy'+vi+'_subsidy');
    setCalc('dy'+vi+'_gross',gross);
    const feedTotal=dynCalcSum('dy'+vi+'_feeds');
    setCalc('dy'+vi+'_feed_total',feedTotal);
    const costs=getVal('dy'+vi+'_heifer')+feedTotal+getVal('dy'+vi+'_milk_mech')+getVal('dy'+vi+'_feed_mech')+getVal('dy'+vi+'_vent')+getVal('dy'+vi+'_manure')+getVal('dy'+vi+'_vet')+getVal('dy'+vi+'_energy')+getVal('dy'+vi+'_insem')+getVal('dy'+vi+'_insure')+getVal('dy'+vi+'_other');
    const md=gross-costs;
    setCalc('dy'+vi+'_costs',costs); setCalc('dy'+vi+'_md',md);
  }
}
...
function recalcDairyProfit() {
  const rev=getVal('dp_milk')*getVal('dp_price')+getVal('dp_sub'), varC=getVal('dp_var');
  const cap=(getVal('dp_mach')+getVal('dp_build'))/2*(getVal('dp_int_own')/100*0.5+getVal('dp_int_hire')/100*0.5);
  const tot=varC+cap+getVal('dp_fixed_other')+getVal('dp_overhead');
  setCalc('dp_total_cost',tot); setCalc('dp_ent_inc',rev-tot); setCalc('dp_profit',rev-tot);
}
